...
- Produces: `YamlPreview({ yaml, filename, onEditedChange })` where
  ...
  - `onEditedChange?: (edited: boolean) => void` — fires `true` once the user manually edits the textarea, so the parent can disable Back. "Reset to generated" restores and fires `false`.
...
```
Note: on the last step, `canContinue` is gated on `!previewEdited` so Finish is disabled once the user has manually edited the YAML (matches the original "Back disabled after edit" intent; here it guards Finish so a hand-edited buffer isn't silently discarded — Download/Copy still act on the buffer). If product prefers Finish always enabled, drop that clause.

...
import { nextName, upsertWithRename, type Action, type ResiliencyState } from './reducer'
    ...
    <div className="policy-sections">
      ...
      {open?.kind === 'timeout' && (
        <TimeoutDialog open editing={!!open.editName} existingNames={Object.keys(pol.timeouts)}
          ...
          onSave={(name, duration) => { upsertWithRename(dispatch, open.editName, name, (from, to) => ({ type: 'RENAME_TIMEOUT', from, to }), { type: 'UPSERT_TIMEOUT', name, duration }); setOpen(null) }} />
      )}
      {open?.kind === 'retry' && (
        <RetryDialog open editing={!!open.editName} existingNames={Object.keys(pol.retries)}
          ...
          onClose={() => setOpen(null)}
          onSave={(name, policy) => { upsertWithRename(dispatch, open.editName, name, (from, to) => ({ type: 'RENAME_RETRY', from, to }), { type: 'UPSERT_RETRY', name, policy }); setOpen(null) }} />
      )}
      {open?.kind === 'cb' && (
        <CircuitBreakerDialog open editing={!!open.editName} existingNames={Object.keys(pol.circuitBreakers)}
          ...
          onClose={() => setOpen(null)}
          onSave={(name, policy) => { upsertWithRename(dispatch, open.editName, name, (from, to) => ({ type: 'RENAME_CB', from, to }), { type: 'UPSERT_CB', name, policy }); setOpen(null) }} />
      )}
      {open?.kind === 'override' && (

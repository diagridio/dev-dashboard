...
describe('StepPolicies', () => {
  ...
  it('suggests the first free timeout name after a deletion instead of colliding', () => {
    const dispatch = vi.fn()
    // timeout1 was deleted; only timeout2 remains — Add must suggest timeout1, not timeout2
    const s = reducer(initialState(), { type: 'UPSERT_TIMEOUT', name: 'timeout2', duration: '60s' })
    render(<StepPolicies state={s} dispatch={dispatch} />)
    fireEvent.click(screen.getByRole('button', { name: /add timeouts/i }))
    expect((screen.getByLabelText(/timeout name/i) as HTMLInputElement).value).toBe('timeout1')
  })
  it('blocks renaming a timeout onto another existing timeout', () => {
    const dispatch = vi.fn()
    let s = reducer(initialState(), { type: 'UPSERT_TIMEOUT', name: 'timeout1', duration: '30s' })
    s = reducer(s, { type: 'UPSERT_TIMEOUT', name: 'timeout2', duration: '60s' })
    render(<StepPolicies state={s} dispatch={dispatch} />)
    fireEvent.click(screen.getByRole('button', { name: /edit timeout1/i }))
  ...
  it('blocks adding a retry whose name duplicates an existing retry', () => {
    const dispatch = vi.fn()
    const s = reducer(initialState(), { type: 'UPSERT_RETRY', name: 'important', policy: { policy: 'constant', duration: '5s' } })
    render(<StepPolicies state={s} dispatch={dispatch} />)
    fireEvent.click(screen.getByRole('button', { name: /add retries/i }))
    fireEvent.change(screen.getByLabelText(/retry name/i), { target: { value: 'important' } })
    expect(screen.getByText(/already exists/i)).toBeInTheDocument()
    expect(screen.getByRole('button', { name: /save/i })).toBeDisabled()
  })
...
describe('StepTargets', () => {
  ...
  it('blocks renaming an app target onto another existing app target', () => {
    const dispatch = vi.fn()
    let s = reducer(initialState(), { type: 'UPSERT_TIMEOUT', name: 'timeout1', duration: '30s' })
    s = reducer(s, { type: 'UPSERT_APP', name: 'orders', target: { timeout: 'timeout1' } })
    s = reducer(s, { type: 'UPSERT_APP', name: 'billing', target: { timeout: 'timeout1' } })
    render(<StepTargets state={s} dispatch={dispatch} />)
    fireEvent.click(screen.getByRole('button', { name: /edit orders/i }))
